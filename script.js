...
function calculate(){
    ...
    var total = document.getEelementById("total");
    ...
    if (isFinite(monthly)){
     ...
     total.innerHTML = ((monthly*payment)-principal).toFixed(2);

     //save the user's input so we can restore it the next time they visit
     save(amount.value, apr.value, years.value, zipcode.value);

     //advitise: find and display local lenders, but ignre network errors 
     try{ //catch any errors that occur within these curly braces 
           getLenders(amount.value, apr.value, zipcode.value);
        }
        catch(e) {/* and those errors*/}
        // Finaly , cahrt loan balance,and equity payments
        chart(principal, interest, monthly, payments);
    }
    else {// Result was Not-a-Number or infinite, which means the input was
        // incomplete or invalid. Clear any previously displayed output.
    
    payment.innerHTML = ""; //erase the content of these elements 
    total.innerHTML="" 
    totalinterest.innerHTML="";
    chart();   //with no argument clear the chart 
    }
}
